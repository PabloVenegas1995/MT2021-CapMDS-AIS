...
bool vertex_set_comparison(set<int> A, set <int> B)
{
    //bool hipo=true;
    ...
    for (int i = 0; i < n_of_vertices; ++i) {
        if (chosen[i]) ++score;
            ...
            if (covered_by[i] == -1) cout << "Node " << i << " is not covered." << endl;
                ...
                if (not chosen[covered_by[i]]) cout << "Node " << i << " is covered by node " << covered_by[i] << " which is NOT chosen for the solution." << endl;
            }
        }
    }
    for (int i = 0; i < n_of_vertices; ++i) {
        if (chosen[i]) {
            if (capacity[i] < number_of_covered[i]) cout << "Node " << i << " has capacity " << capacity[i] << " and covers " << number_of_covered[i] << " nodes." << endl;
        }
    }
}
    ...
    while (num_nodes_uncovered > 0) {
        ...
        if (capacity[chosen_vertex] >= int(uncovered_neighbors[chosen_vertex].size())) {
            ...
            for (set<int>::iterator sit = to_cover.begin(); sit != to_cover.end(); ++sit) {
                ...
                for (set<int>::iterator ssit = adj[*sit].begin(); ssit != adj[*sit].end(); ssit++) uncovered_neighbors[*ssit].erase(*sit);
            }
            ...
            for (int cc = 0; cc < capacity[chosen_vertex]; ++cc) {
                ...
                for (set<int>::iterator ssit = adj[el].begin(); ssit != adj[el].end(); ssit++) uncovered_neighbors[*ssit].erase(el);
            }
        }

        for (set<int>::iterator sit = adj[chosen_vertex].begin(); sit != adj[chosen_vertex].end(); sit++) uncovered_neighbors[*sit].erase(chosen_vertex);
  ...
  //for (const auto & entry : fs::directory_iterator(file_directory)) {
       ...
       // }
        ...
        while (ctime < computation_time_limit) {
            ...
            if(vertex_set_comparison(vertex_in_c,vertex_in_c_old)) {

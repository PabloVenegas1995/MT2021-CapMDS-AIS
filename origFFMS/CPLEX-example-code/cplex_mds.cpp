/***************************************************************************
                        cplex_mds.cpp  -  description
                             -------------------
    program use          : CPLEX code for solving the "minimum dominating set" problem
    begin                : Wed Sep 15 2021
...
void run_cplex(clock_t& start) {

    // variables for storing the result, the computation time, and the optimality gap obtained by CPLEX for the provided input file (within the allowed computation time)
    ...
    try{
        ...
        // create an array of binary variables, one for each node of the input graph
        ...
        // add a minimization function to the model. This function minimizes the expression generated before
        ...
        // redirect all warnings to the null stream in order to avoid printing them to the screen
        cpl.setWarning(env.getNullStream());
        cpl.use(loggingCallback(env, start, time_stamp, result, gap));

        // call solve() in order to start solving the problem
        cpl.solve();
    
        // after solving, check if the the solver has derived an optimal solution, or at least a feasible one.
        // Finally, read out the values of the decision variables correponding to the best solution that was found and write the node/vertex indices of the nodes/vertices from the best solution to the screen
        if (cpl.getStatus() == IloAlgorithm::Optimal or cpl.getStatus() == IloAlgorithm::Feasible) {
            ...
            // Reading out the values of the decision variables correponding to the best solution that was found
